...
export const getRealEstateAudit = (yearsOwned, cagr, realTotalYield, roe, isUnderConstruction, isRented) => {
  const isHighGrowth = cagr >= 10;

  // 1. если объект строится
  if (isUnderConstruction) {
    if (isHighGrowth) {
      return {
        level: 'green',
        status: 'Инвестиционная фаза',
  ...
  // 2. жесткое правило для сданных объектов: глубокий убыток
  if (realTotalYield <= -15) {
    return {
      level: 'red',
      status: 'Срочно к продаже',
      comment: 'Объект генерирует глубокий убыток (полная реальная доходность ниже -15%). Готовый актив не должен так тянуть капитал вниз. Срочно перевложить'
    };
  }

  // 3. сдан, срок владения 5 лет и более
  if (yearsOwned >= 5) {
    if (!isHighGrowth) {
      return {
        level: 'red',
  ...
  // 4. сдан, срок владения 4 года (готовимся к продаже)
  if (yearsOwned === 4) {
    return {
      level: 'orange',
      status: 'Пора перевложить',
  ...
  // 5. сдан, срок до 4 лет, низкий рост
  if (!isHighGrowth) {
    return {
      level: 'orange',
  ...
  // 6. сдан, срок до 4 лет, хороший рост, но не сдается
  if (!isRented) {
    return {
      level: 'orange',
  ...
  // 7. сдан, хороший рост, сдается в аренду (идеальный актив)
  return {
    level: 'green',
