...
export const LeadModal = ({ isOpen, onClose }) => {
  ...
  const isSubmitDisabled = !name || !phone || !agreePrivacy || !agreeData || !agreeMarketing || isLoading;
  ...
  const handleSubmit = async (e) => {
    ...
    setTimeout(() => {
      ...
      // Автоматическое закрытие формы через 3 секунды после успеха
      setTimeout(() => {
        setIsSuccess(false); // Сбрасываем статус
